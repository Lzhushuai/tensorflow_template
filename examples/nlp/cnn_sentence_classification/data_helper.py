...
def get_dataset(data_dir='../../data/sentence', test_percentage=0.2):
    pos_path = os.path.join(data_dir, 'rt-polarity.pos')
    neg_path = os.path.join(data_dir, 'rt-polarity.neg')
    x_, y = load_data_and_labels(pos_path, neg_path)

    # Build vocabulary
    max_document_length = max([len(x.split(" ")) for x in x_])
    tf.logging.debug("max_document_length = %i" % max_document_length)
    ...
    tf.logging.info("Train/Test split: {:d}/{:d}".format(len(y_train), len(y_test)))
